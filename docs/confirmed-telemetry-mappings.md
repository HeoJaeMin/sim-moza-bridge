...
- `output-target`: 네이티브 원본 입력과 MOZA 키 지원은 확인됐지만 Pit House 출력 연결은 아직 구현되지 않음
...
현재 Windows 어댑터와 별도 웹 대시보드는 `LMU_Data` 원본을 읽어 로컬 HUD, 저장과 분석에 사용합니다. 다만 이 경로를 Pit House 키로 직접 출력하는 연결은 아직 구현하지 않았습니다. 아래 표는 MOZA의 `Le mans ultimate` 컬럼에서 지원이 확인된 출력 대상이며, 실제 원본 필드와의 대응은 별도로 검증해야 합니다.
...
3. LU/LMU는 구현된 `LMU_Data` 입력과 확인된 `output-target` 키를 대조하고, 검증된 필드부터 Pit House 출력 연결을 별도로 구현합니다.
