...
현재 일반 실행 CLI는 F1 25 -> MOZA 전달에 집중하므로 CSV 로깅과 분석 옵션은 노출하지 않습니다. macOS/Windows에서는 native HUD가 기본 실행 때 자동으로 올라오며 브라우저를 열지 않습니다.
...
그 외 플랫폼에서는 기존 HTTP HUD가 fallback으로 남아 있으며 `http://127.0.0.1:8765`를 기본 브라우저로 엽니다.
